...
class _ExpensePageState extends State<ExpensePage> {
  ...
  Future<void> _load() async {
    ...
    final total = await _dao.getTotal(type: 'expense', startDate: _shiftStart);
    if (mounted) {
      setState(() {
        _list = list;
        _total = total;
    ...
    final saved = await showConstrainedDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: Text(existing != null ? 'Edit Pengeluaran' : 'Tambah Pengeluaran'),
          content: Form(
            key: formKey,
  ...
  Future<void> _delete(Transaction t) async {
    final ok = await showConstrainedDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Hapus pengeluaran "${t.category}" sebesar Rp ${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(t.amount)}?'),
        actions: [
          TextButton(
                  ...
                  child: _list.isEmpty
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              Icon(Icons.trending_down, size: 48, color: Theme.of(context).colorScheme.onSurfaceVariant),
                              const SizedBox(height: 8),
                              Text('Belum ada pengeluaran', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                            ],
                          ),
                        )
                      : RefreshIndicator(
